...
class GoogleSheetsAPI {
    constructor() {
        this.apiUrl = "https://script.google.com/macros/s/AKfycbwzwvLo150_T7h8Z-AEeDGhy8tIQx9DVtFyGOiMRxC37piWvIZvKfGo5cSEhG4sev9bNQ/exec";
        this.cache = new Map();
...
// =============================
// Helper: Get user-facing mode (inverted)
// =============================
function getUserMode(adminMode) {
    if (adminMode === 'to give') return 'to get';
    if (adminMode === 'to get') return 'to give';
    return adminMode;
}

// =============================
// Helper: Get admin mode from user mode
// =============================
function getAdminMode(userMode) {
    if (userMode === 'to give') return 'to get';
    if (userMode === 'to get') return 'to give';
    return userMode;
}
...
async function loadAdminUsers() {
    ...
    const summaryContainer = document.getElementById('adminUserSummary');
    ...
    try {
        ...
        usersWithTransactions.forEach(user => {
            user.transactions.forEach(t => {
                ...
                // For users, the mode is inverted
                const userMode = getUserMode(adminMode);
...
async function submitAddTransaction(event) {
    ...
    try {
        ...
        const adminMode = document.getElementById('transactionMode').value;
        const amount = document.getElementById('transactionAmount').value;
        const date = document.getElementById('transactionDate').value;
        const fileInput = document.getElementById('billUpload');

        if (!title || !adminMode || !amount || !date) {
        ...
        if (result && (result.success || result.message?.includes('Success'))) {
            // When adding to users, the mode is inverted
            const userMode = getUserMode(adminMode);
            await api.addTransactionToAllUsers(transactionId, title, userMode, amount, billUrl, date);
...
async function submitEditTransaction(event) {
    ...
    try {
        ...
        const adminMode = document.getElementById('editTransactionMode').value;
        ...
        const result = await api.updateTransaction(transactionId, title, adminMode, amount, billUrl, date);

        if (result && result.success) {
            // Also update user transactions with inverted mode
            const userMode = getUserMode(adminMode);
            // We need to update all users' transactions
            const users = await api.getSheet('user_credentials');
            if (users && Array.isArray(users)) {
                for (const user of users) {
                    const username = user.username;
                    if (!username) continue;
                    const userTxn = await api.getUserTransactions(username);
                    if (userTxn && Array.isArray(userTxn)) {
                        const txn = userTxn.find(t => t.transaction_id === transactionId);
                        if (txn) {
                            await api.updateUserTransaction(username, transactionId, userMode, txn.status || 'pending', txn.amount || amount, txn.date || date);
                        }
                    }
                }
            }
...
async function openUserTransactionEditModal(username, transactionId) {
    ...
    try {
        ...
        // For admin editing, show the user-facing mode
        const userMode = userTxn.mode || 'to get';
        ...
        document.getElementById('userEditMode').value = userMode;
...
async function submitUserTransactionEdit(event) {
    ...
    try {
        ...
        const userMode = document.getElementById('userEditMode').value;
        ...
        if (!userMode || !status || !amount || !date) {
            showUserEditError('Please fill in all required fields');
            return;
        }

        // When storing, we need to invert back to admin mode
        const adminMode = getAdminMode(userMode);

        const result = await api.updateUserTransaction(username, transactionId, adminMode, status, amount, date);
